...
% plot the Wavelets for both conditions side by size for channel 27
condition1WaveletData = squeeze(WAV.data(27,:,:,1));
condition2WaveletData = squeeze(WAV.data(27,:,:,2));
subplot(1,2,1);
imagesc(condition1WaveletData);
title('Channel Fz: Condition One');
xlabel('Time (ms)');
ylabel('Frequency (Hz)');
set(gca,'YDir','normal');
...
title('Channel Fz: Condition Two');
xlabel('Time (ms)');
set(gca,'YDir','normal');
ylabel('Frequency (Hz)');
xticklabels = EEG.times(1):100:EEG.times(end);
